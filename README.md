...
Здесь собраны самые популярные вопросы, задаваемые на собеседованиях front-end разработчиков на React.js. Тематика вопросов включает в себя как основы JavaScript и веб-технологий так и глубокое понимание работы React.js и смежных технологий (Redux, MobX и прочего).
...
<div>
  <p>В глобальном контексте выполнения (за пределами каких-либо функций), this ссылается на глобальный объект window, а в строгом режиме значение this остается undefined.</p>
...
<div>
  ...
  <p>
    WeakSet и WeakMap - это специальные структуры данных в JavaScript, которые могут использоваться для хранения "вспомогательных" данных, связанных с объектами,  Они позволяют автоматически удалить данные,   
    связанные с объектами, когда объекты больше не имеют других активных ссылок на них. Когда все ссылки на объекты, связанные с данными в WeakSet или WeakMap, становятся недоступными и объекты становятся 
    недостижимыми из вашего кода, сборщик мусора JavaScript может удалить эти объекты и соответствующие данные из памяти.
    Примеры использования:
    WeakSet: Предположим, у вас есть список элементов на веб-странице, и вы хотите добавить обработчики событий к ним. Вы можете использовать WeakSet, чтобы хранить ссылки на элементы и автоматически удалять 
    обработчики, когда элементы удаляются из DOM.
  </p>
  <p>
    ...
    // Позже, если элемент будет удален из DOM, обработчик также будет удален из памяти.
  </p>

  WeakMap: Предположим, у вас есть объекты, представляющие пользователей, и вы хотите связать с ними дополнительные данные. Вы можете использовать WeakMap, чтобы связать объекты пользователя
  с этими данными и автоматически удалить данные, когда объект пользователя больше не используется.
  ...
  <p>
    ...
    const userDetails = { age: 30, email: 'john@example.com' };

    userData.set(user, userDetails);

    // Позже, если объект пользователя больше не используется, userDetails также будет удален из памяти.

  </p>

  Итак, WeakSet и WeakMap полезны для хранения "слабых" связей между объектами, где дополнительные данные связаны с объектами, Таким образом, WeakSet и WeakMap позволяют автоматически удалять связанные с ними 
  данные, когда объекты больше не используются и могут быть удалены сборщиком мусора.
